            ...
            <Grid item xs={12} md={4}>
              <Paper elevation={2}>
                ...
                <List>
                  {
                    topics.length > 0
                      ? topics.map(topic => <TopicItem topic={topic} key={topic.id} onClick={() => this.goToTopic}/>)
            ...
            <Grid item xs={12} md={4}>
              <Paper elevation={2}>
                ...
                <List>
                  {
                    replies.length > 0
                      ? replies.map(topic => <TopicItem topic={topic} key={topic.id} onClick={() => this.goToTopic(topic.id)}/>)
                      : <Typography align="center">
                        最近没有新的回复
                      </Typography>
                  }
                ...
                <List>
                  {
                    collections.length > 0
                      ? collections.map(topic => <TopicItem topic={topic} key={topic.id} onClick={() => this.goToTopic(topic.id)}/>)
                      : <Typography align="center">
                        还么有收藏话题哦
                      </Typography>
                  }
